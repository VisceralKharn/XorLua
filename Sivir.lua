...
local function Use_Q(target)
    if Local_spellbook:get_spell_slot( spell_slot_t.q ):is_ready() and globals.get_game_time() > Spell_limiter_q then
        local pred_pos = target:get_predicted_position( Local_hero:get_position() , 1350, 1200, 120, 0.25 )
...
local function Use_W(target)
    if Local_spellbook:get_spell_slot(spell_slot_t.w):is_ready() and globals.get_game_time() > Spell_limiter_w then
        local pred_pos = target:get_predicted_position(Local_hero:get_position(),0,500,0,0)
        if pred_pos:length() > 1 then
...
local function Combo()
    ...
    if orbwalker_target ~= -1 then
        ...
        if Menu.combo_use_q:get_value() and Local_hero:get_mana() > max_mana * (Menu.combo_mana_q:get_value()/100) and Local_hero:get_mana() > 60 then 
            Use_Q(target) 
        end
        if Menu.combo_use_w:get_value() and Local_hero:get_mana() > max_mana * (Menu.combo_mana_w:get_value()/100) and Local_hero:get_mana() > 60 then 
            Use_W(target)
        end
    end
end
